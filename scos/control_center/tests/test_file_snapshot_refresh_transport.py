...
def test_no_forbidden_runtime_source_markers_and_no_frontend_route_files() -> None:
    ...
    # Cohort 9A introduced exactly ONE reviewed, read-only, local-only transport
    # boundary: a GET-only Next.js route handler that performs a single
    # fs.readFileSync on a pre-generated committed snapshot artifact. No mutation
    # method, no subprocess, no secret exposure, no external egress. Every other
    # route.ts (middleware, server actions, etc.) remains forbidden. The .next
    # build-output directory is generated by `next build` and is not source.
    _REVIEWED_READ_ONLY_ROUTE = Path(
        "apps/control-center/app/api/control-center-snapshot/route.ts"
    )
    route_files = [
        p
        for p in Path("apps/control-center").rglob("route.ts")
        if p != _REVIEWED_READ_ONLY_ROUTE and ".next" not in p.parts
    ]
