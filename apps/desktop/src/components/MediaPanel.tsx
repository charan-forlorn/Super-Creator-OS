...
import { selectMediaFile, probeMedia, generateThumbnail, generatePreviewProxy } from "../bridge";
...
export function MediaPanel() {
  ...
  async function importFile() {
    ...
    try {
      ...
      // Non-blocking thumbnail (fire and forget).
      try {
        const out = `${path}.thumb.png`;
        await generateThumbnail(path, out, Math.min(1, probe.durationSec / 2));
        setThumbnail(probe.id, out);
      } catch {
        /* thumbnail optional */
      }

      // ROOT_CAUSE_3: deterministically decide whether the original can be
      // previewed directly, or a cached H.264/AAC proxy is required.
      if (previewNeedsProxy({ kind: probe.kind as any, videoCodec: probe.videoCodec, audioCodec: probe.audioCodec })) {
        try {
          const proxyOut = `${path}.proxy.mp4`;
          await generatePreviewProxy(path, proxyOut);
