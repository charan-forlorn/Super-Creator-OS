/**
 * Cohort 10E — HVS render projection (GET, read-only bridge).
 *
 * Same-origin, local-first read boundary. Returns the authoritative render
 * truth state + the deterministic plan for a prepared project, produced
 * ENTIRELY by the Python authority through the bridge. No mutation, no HVS
 * call, no external network, no browser-supplied path.
 */
