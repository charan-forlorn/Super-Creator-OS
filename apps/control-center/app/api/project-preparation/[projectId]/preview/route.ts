/**
 * Cohort 10C — project-preparation create preparation preview (POST).
 *
 * Same-origin, local-first mutation boundary. Creates a dry-run
 * preparation preview (PREPARATION_PREVIEW_READY) for an approved
 * project through the authorative locked store (mirrors
 * scos/control_center/solo_project_preparation.py). The preview is
 * metadata ONLY: side_effects_performed / render_started /
 * hvs_project_created are ALWAYS false — no HVS project, no render
 * artifact, no upload, no publish. Idempotent on replay; rejects a
 * stale revision (REVISION_CONFLICT); rejects an unknown project;
 * fail-closed on corrupt/unavailable store.
 *
 * NEVER writes memory/database.json, NEVER initializes HVS, NEVER
 * starts a render, NEVER reaches external network, NEVER executes a
 * subprocess, NEVER trusts a browser-supplied path.
 */
...
function storePath(): string {
  ...
  // app/api/project-preparation/[projectId]/preview/route.ts -> memory/runtime/control-center/
...
export async function POST(
  ...
  const result = store.createPreview(projectId, expectedRevision);
  if (!result.ok) {
