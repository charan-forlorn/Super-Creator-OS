import { parseAiPlan, AiPlanValidationError } from "@haios/ai-core";
import { ProviderUnavailableError, AI_PLAN_SYSTEM_PROMPT, } from "./types.js";
/** Extract the first balanced JSON object from arbitrary model text. */
export function extractJsonObject(text) {
    const start = text.indexOf("{");
    if (start === -1)
        throw new AiPlanValidationError("no JSON object in model output");
    let depth = 0;
    let inStr = false;
    let esc = false;
...
export function modelTextToPlan(rawText, _provider) {
...
function routeOfflineInstruction(text, selectedClipId) {
    const t = text.toLowerCase();
    const target = selectedClipId
        ? { kind: "clip", clipId: selectedClipId }
        : { kind: "selection" };
...
export class OllamaProvider {
    ...
    async generate(req) {
        if (req.signal?.aborted)
            throw new ProviderUnavailableError("ollama", "aborted");
        const f = this.cfg.fetchImpl ?? fetch;
        let res;
        try {
        ...
        if (!res.ok)
            throw new ProviderUnavailableError("ollama", `status ${res.status}`);
...
export class OpenAICompatibleProvider {
    ...
    async generate(req) {
        if (req.signal?.aborted)
            throw new ProviderUnavailableError("openai-compatible", "aborted");
        const f = this.cfg.fetchImpl ?? fetch;
        let res;
        try {
        ...
        if (!res.ok)
            throw new ProviderUnavailableError("openai-compatible", `status ${res.status}`);
        const data = (await res.json());
        return {
