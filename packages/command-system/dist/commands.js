...
function assertTrackUnlocked(track) {
    if (track.locked)
        throw new CommandError(`TRACK_LOCKED: ${track.id}`);
}
...
function transitionPredecessors(project) {
...
function assertTransitionTopology(before, after, selectedIds) {
...
function restoreCrossTrackSnapshot(prev, snapshot) {
    const current = { tracks: prev.tracks, durationSec: prev.durationSec };
    return {
...
export const moveAcrossTracksCommand = {
    type: MOVE_CLIP_ACROSS_TRACKS,
    schema: moveAcrossTracksSchema,
    execute(prev, { moves }) {
        const selectedIds = new Set();
        const resolved = [];
        const trackIdCounts = new Map();
        for (const track of prev.tracks)
            trackIdCounts.set(track.id, (trackIdCounts.get(track.id) ?? 0) + 1);
        const assertReferencedTrackIdUnique = (trackId) => {
            if ((trackIdCounts.get(trackId) ?? 0) > 1)
                throw new CommandError(`CROSS_TRACK_TRACK_ID_AMBIGUOUS: ${trackId}`);
        };
        ...
        for (const move of moves) {
            ...
            if (selectedIds.has(move.clipId))
                throw new CommandError(`CROSS_TRACK_DUPLICATE_CLIP_ID: ${move.clipId}`);
            selectedIds.add(move.clipId);
            const containments = prev.tracks.flatMap((track) => track.clips.filter((clip) => clip.id === move.clipId).map((clip) => ({ clip, track })));
            if (containments.length > 1)
                throw new CommandError(`CROSS_TRACK_CLIP_ID_AMBIGUOUS: ${move.clipId}`);
            let found;
            try {
            ...
            if (source.kind !== "video" && source.kind !== "audio")
                throw new CommandError(`CROSS_TRACK_TRACK_KIND_UNSUPPORTED: ${source.kind}`);
            if (selectionKind !== null && source.kind !== selectionKind)
                throw new CommandError("CROSS_TRACK_SELECTION_KIND_MISMATCH");
            ...
            if (!target)
                throw new CommandError(`CROSS_TRACK_TARGET_TRACK_NOT_FOUND: ${move.targetTrackId}`);
            if (target.kind !== source.kind)
                throw new CommandError(`CROSS_TRACK_TARGET_TRACK_KIND_MISMATCH: ${move.targetTrackId}`);
        ...
        const snapshot = { tracks: prev.tracks, durationSec: prev.durationSec };
        const next = { ...prev, tracks, durationSec: Math.max(recomputeDuration({ ...prev, tracks }), prev.durationSec), updatedAt: new Date().toISOString() };
...
export const placeCaptionCommand = {
    ...
    execute(prev, { text, start, duration, targetTrackId }) {
        ...
        if (targetTrackId) {
            ...
            if (!target)
                throw new CommandError(`CAPTION_TARGET_TRACK_NOT_FOUND: ${targetTrackId}`);
            if (target.kind !== "text")
                throw new CommandError(`CAPTION_TARGET_TRACK_KIND_MISMATCH: ${target.kind}`);
            existing = target;
        }
...
export const addCaptionCommand = {
    ...
    execute(prev, { caption }) {
        ...
        if (targetTrack.kind !== "text")
            throw new CommandError(`CAPTION_TRACK_KIND_MISMATCH: ${targetTrack.kind}`);
...
export const reorderTrackCommand = {
    type: REORDER_TRACK,
    ...
    execute(prev, { trackId, toIndex }) {
        ...
        if (toIndex >= prev.tracks.length)
            throw new CommandError(`TRACK_INDEX_OUT_OF_RANGE: ${toIndex}`);
        if (fromIndex === toIndex)
            return { next: prev, inverse: reorderTrackCommand };
...
export const setTrackControlsCommand = {
...
function resolveMediaTargetTrack(project, kind, targetTrackId, createTrackId) {
    if (targetTrackId) {
        const track = project.tracks.find((candidate) => candidate.id === targetTrackId);
        if (!track)
            throw new CommandError(`TIMELINE_TARGET_TRACK_NOT_FOUND: ${targetTrackId}`);
        if (track.kind !== kind)
            throw new CommandError(`TIMELINE_TARGET_TRACK_KIND_MISMATCH: ${track.kind} != ${kind}`);
        assertTrackUnlocked(track);
        return { tracks: project.tracks, track, created: false };
    }
    const compatible = project.tracks.filter((candidate) => candidate.kind === kind);
    if (compatible.length > 1)
        throw new CommandError(`TIMELINE_TARGET_TRACK_REQUIRED: ${kind}`);
    if (compatible.length === 1) {
...
export const placeProbedMediaCommand = {
    ...
    execute(prev, { probe, place, targetTrackId }) {
        ...
        if (place) {
            ...
            if (resolved.created)
                createdTrackId = resolved.track.id;
        }
        let next = {
            ...prev,
            assets: [...prev.assets, asset],
        ...
        const inverse = {
            ...
            execute(p, { probe: pr, place: pl }) {
                let tracks = p.tracks;
                if (createdTrackId)
                    tracks = tracks.filter((track) => track.id !== createdTrackId);
...
function ensureTimelineTrack(project, asset, targetTrackId) {
