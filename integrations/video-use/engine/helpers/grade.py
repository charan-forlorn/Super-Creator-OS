...
def _sample_frame_stats(
    ...
    # [super-creator-os integration] Windows-compat: ffmpeg's filtergraph cannot
    # parse an ABSOLUTE Windows path in `metadata=print:file=` — the drive colon
    # (C:\) is read as a filter option separator and NO escaping (\:, forward
    # slashes) fixes it. A RELATIVE basename in the cwd parses cleanly on both
    # Windows and POSIX, so we write the metadata file next to the cwd and remove
    # it in `finally`. (uuid avoids collisions across parallel auto-grade calls.)
    metadata_path = f"_scos_signalstats_{uuid.uuid4().hex[:10]}.txt"
